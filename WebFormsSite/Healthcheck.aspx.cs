...
public partial class Healthcheck : System.Web.UI.Page
{
    ...
    {
        var retVal = CheckDbConnection("DB1");
        var retVal2 = CheckDbConnection("DB2");

        if (retVal != string.Empty && retVal2 != string.Empty)
        {
            retVal += "<hr />" + retVal2;
        }
        retVal2 = CheckDbConnection("DB3");

        if (retVal != string.Empty & retVal2 != string.Empty)
        {
            retVal += "<hr />" + retVal2 + "<hr />";
        }
        if (retVal == string.Empty && retVal2 == string.Empty) {
            retVal = "OK";
        }
    ...
    protected string CheckDbConnection(string connStrKey)
    {
        ...
        {
            ItemFailed = true;
            return String.Format("Connection Key {0}, Sql Connection Error: {1} <br />", connStrKey, sqlE.Message);
        }
        finally
        {
    ...
    protected void CheckDbStoredProc()
    {
        ...
        spResult = Database.SqlStoredProcedureAccessCheck(Helper.GetConnString(Db2ConStrKey), "uspExerciseDetail", "@workoutExerciseId", 555);

        if (spResult.Item2)
        {
            Sb.Append(HtmlTableRow(true, "Execute DB2.dbo.uspExerciseDetail", "OK"));
        }
        else
        {
            Sb.Append(HtmlTableRow(false, "Execute DB2.dbo.uspExerciseDetail", spResult.Item1));
            ItemFailed = true;
        }

        spResult = Database.SqlStoredProcedureAccessCheck(Helper.GetConnString(Db3ConStrKey), "WalkingProgramSelect", "@ID", 1);

        if (spResult.Item2)
        {
            Sb.Append(HtmlTableRow(true, "Execute DB3.dbo.WalkingProgramSelect", "OK"));
        }
        else
        {
            Sb.Append(HtmlTableRow(false, "Execute DB3.dbo.WalkingProgramSelect", spResult.Item1));
            ItemFailed = true;
        }
