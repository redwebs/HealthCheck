...
public static class DbHelper
{
    ...
    public static Tuple<string, bool> SqlStoredProcedureAccessCheck(string connString, string storedProcName, string paramName, int paramVal)
    {
        ...
        using (var sqlConn = new SqlConnection(connString))
        {
            ...
            {
                string msg = String.Format("Error executing Stored Proc: {0}, Message: {1}", storedProcName, ex.Message);
                Log.Error(msg);
    ...
    public static bool SqlStoredProcExists(string database, string procedureName)
    {
        ...
        using (var conn = new SqlConnection(GetConnStrForDb(ConfigurationManager.ConnectionStrings[database].ConnectionString)))
        {
            conn.Open();
