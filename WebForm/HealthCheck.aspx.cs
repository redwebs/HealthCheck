...
namespace WebForm
{
    ...
    protected const string Db2ConStrKey = "DB2";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                // get a simple one line version of the app
                GetVersionInfo();
        ...
        protected string DetailCheck()
        {
            ...
            var endPoint = SiteSettings.MightySiteSSOurl;
            ...
            ListAppSettings();
            GetVersionInfoWebApp();
            GetVersionInfoWebSite();
            TimeZoneInformation();
        ...
        {
            var spResult = DbHelper.SqlStoredProcedureAccessCheck(Helper.GetConnString(Db1ConStrKey),
                "GetSubjectBedTime", "@uid", 1);
            ...
            var dataTable = DbHelper.GetDataTableConStrKey(Db1ConStrKey,
    ...
    public void GetVersionInfo()
    {
        ...
        public void GetVersionInfoWebSite()
        {
            ...
            Sb.Append(HtmlTableTop("Version From App_GlobalResources/About.resx"));

            var globalResourceObject = GetGlobalResourceObject("About", "Major");
            Sb.Append(HtmlTableRow(true, "Major ",
                (globalResourceObject == null) ? missingMsg : globalResourceObject.ToString()));

            globalResourceObject = GetGlobalResourceObject("About", "Minor");
            Sb.Append(HtmlTableRow(true, "Minor ",
                (globalResourceObject == null) ? missingMsg : globalResourceObject.ToString()));

            globalResourceObject = GetGlobalResourceObject("About", "Patch");
            Sb.Append(HtmlTableRow(true, "Patch ",
                (globalResourceObject == null) ? missingMsg : globalResourceObject.ToString()));

            globalResourceObject = GetGlobalResourceObject("About", "Build");
            Sb.Append(HtmlTableRow(true, "Build ",
                (globalResourceObject == null) ? missingMsg : globalResourceObject.ToString()));

            globalResourceObject = GetGlobalResourceObject("About", "Increment");
            Sb.Append(HtmlTableRow(true, "Increment ",
                (globalResourceObject == null) ? missingMsg : globalResourceObject.ToString()));
            Sb.Append(HtmlTableBottom);
        }

        public void GetVersionInfoWebApp()
        {
            const string missingMsg = "Not Found";
            Sb.Append(HtmlTableTop("Application Version Info"));

            var version = Assembly.GetExecutingAssembly().GetName().Version;

            if (version == null)
            {
                Sb.Append(HtmlTableRow(true, "Version ", missingMsg));
                return;
            }

            Sb.Append(HtmlTableRow(true, "Version ", version.ToString()));
            Sb.Append(HtmlTableRow(true, "Major ", version.Major.ToString()));
            Sb.Append(HtmlTableRow(true, "Minor ", version.Minor.ToString()));
            Sb.Append(HtmlTableRow(true, "Build ", version.Build.ToString()));
            Sb.Append(HtmlTableRow(true, "Revison ", version.Revision.ToString()));
            Sb.Append(HtmlTableBottom);
        }
        ...
        // These are not called or displayed above, but are a template for displaying specific data from a DbHelper

        public int GetEmailQueue()
        {
            var dt = DbHelper.GetDataTableConStrKey(Db1ConStrKey,
                "Select count(*) 'Queued' from EmailQueue where nextProcessingTime < getdate() ");
